      ...
      >
        ...
        <div className="chat-info">
          <div className="chat-name">
            {chatDisplayName}
            {isMuted && <span className="chat-muted-indicator" title="Muted">🔕</span>}
          </div>
        ...
        <div className="chat-meta">
          ...
          {chat.unreadCount > 0 && (
            <div className="unread-count">{chat.unreadCount > 99 ? '99+' : chat.unreadCount}</div>
          )}
        </div>
        <button
          type="button"
        ...
        <div className="sidebar-header">
          <div className="sidebar-header-top">
            <span className="sidebar-title">
              Messages
              {totalUnread > 0 && (
                <span className="sidebar-unread-total" title={totalUnread + " unread"}>{totalUnread > 99 ? '99+' : totalUnread}</span>
              )}
            </span>
          </div>
                ...
                <li className="sidebar-empty-state">
                  {searchQuery ? (
                    <>
                      ...
                      <span className="empty-title">No results for "{searchQuery}"</span>
                      <span className="empty-hint">Try a different name or keyword</span>
                    </>
                  ) : (
                    <>
                      <span className="empty-icon">&#x1F4AC;</span>
                      <span className="empty-title">No chats yet</span>
                      <span className="empty-hint">Start a new conversation below</span>
                    </>
                  )}
                </li>
              ) : (
